...
int main(int argc, const char* argv[]){
  ...
  if(!strcmp(role, "server")){
    ...
    char *encapsulated_key, *key = malloc(16);
    receive(encapsulated_key, server_addr, NULL);
    size_t out_len = 16, in_len = strlen(encapsulated_key);
    EVP_PKEY_CTX *ctx = EVP_PKEY_CTX_new(client_key, NULL);
    EVP_PKEY_decrypt_init(ctx);
    EVP_PKEY_decrypt(ctx, key, out_len, encapsulated_key, in_len);
    ps(key);
    BIO_free(b);
  }
  err:
